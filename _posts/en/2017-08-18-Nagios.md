...
Custom service checks are defined in two parts: A mandatory definition in the Nagios server, and an optional second definition in the monitored device.
    ...
    - In the Nagios server, we have to define the custom command name that will be run through the agent in the monitored device.
    - In the monitored device, the definition of the custom command itself.
